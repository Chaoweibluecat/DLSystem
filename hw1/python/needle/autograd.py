...
def compute_gradient_of_variables(output_tensor, out_grad):
    ...
    dict_grad = {reverse_topo_order[0]: out_grad}
    for node in reverse_topo_order:
        node.grad = dict_grad.get(node, None)
        node: Tensor = node
        # 叶子节点
        if node.inputs is None or len(node.inputs) == 0:
            continue
        ij_grad = node.op.gradient_as_tuple(node.grad, node)
        for i, input_node in enumerate(node.inputs):
            if not input_node.requires_grad:
                continue
            if input_node not in dict_grad:
                dict_grad[input_node] = ij_grad[i]
            else:
                dict_grad[input_node] = dict_grad[input_node] + ij_grad[i]
