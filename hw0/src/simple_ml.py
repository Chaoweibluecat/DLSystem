...
def softmax_regression_epoch(X, y, theta, lr=0.1, batch=100):
    ...
    for start in range(0, num_examples, batch):
        ...
        y_batch = y[start:end]
        # one-hot encoding
        y_oh = np.eye(num_classes, dtype=np.float32)[y_batch]
        forward = X_batch @ theta  # (num_examples, num_classes)
        # softmax CEL grad => p - y_oh
        p = np.exp(forward) / np.sum(np.exp(forward), axis=1, keepdims=True)
        grad_sfm = p - y_oh  # softmax grad
        # 外积 >> input_dim 列向量 外积 num_classes 行向量 => theta grad
        grad = X_batch.T @ grad_sfm / X_batch.shape[0]
        # (input_dim, num_classes)
        theta -= lr * grad  # update theta in place
...
def nn_epoch(X, y, W1, W2, lr=0.1, batch=100):
    ...
    for start in range(0, num_examples, batch):
        ...
        y_batch = y[start:end]
        hidden = X_batch @ W1
        hidden_relu = np.maximum(hidden, 0)  # ReLU activation
        logits = hidden_relu @ W2  # (num_examples, num_classes)
        # softmax CEL grad => p - y_oh
        y_oh = np.eye(num_classes, dtype=np.float32)[y_batch]
        p = np.exp(logits) / np.sum(np.exp(logits), axis=1, keepdims=True)
        grad_sfm = p - y_oh  # softmax
        grad_hidden = grad_sfm @ W2.T
        grad_hidden[hidden_relu <= 0] = 0  # ReLU backprop
        # Update weights
        W1 -= lr * (X_batch.T @ grad_hidden) / X_batch.shape[0]
        W2 -= lr * (hidden_relu.T @ grad_sfm) / hidden_relu.shape[0]
